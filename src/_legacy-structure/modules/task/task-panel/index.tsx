...
} from "@/_legacy-structure/common/components/title-content-fields"
import { panel } from "@/_legacy-structure/common/components/class-names"
import { Icon } from "~/smui/icon/components"
import { parseTaskUpdateInput, Task } from "@/database/models/task"
import { db } from "@/database/db-client"
import { useAuth } from "@/_legacy-structure/modules/auth/use-auth"
import { Button } from "~/smui/button/components"
import { Tooltip, TooltipTrigger } from "~/smui/tooltip/components"
import { formatDate } from "@/_legacy-structure/common/utils/date-utils"
...
export function TaskPanel({ task }: { task: Task }) {
  ...
  const showTaskAge =
    (!task.status_time || !isSameMinute(task.status_time, task.created_at)) &&
    task.status !== "done"
    ...
    <div className={base()}>
      <div className={section()}>
        <div className="flex w-full items-center gap-4 overflow-x-auto">
          ...
          <span className={"text-neutral-text min-w-fit text-sm font-medium"}>{statusText}</span>
          <div className="grow" />
          {showTaskAge && (
            <TooltipTrigger delay={0} closeDelay={0}>
              <Focusable>
                <span className="text-neutral-muted-text min-w-fit cursor-default text-sm">
                  Created {taskAge} ago
                </span>
              </Focusable>
              <Tooltip
      ...
      />
      <div className={section({ className: "flex items-center gap-8" })}>
